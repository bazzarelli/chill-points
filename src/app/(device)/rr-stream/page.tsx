...
import Battery from "@/app/components/heart/Battery";
import DeviceDesc from "@/app/components/heart/DeviceDesc";
import HeartRate from "@/app/components/heart/HeartRate";
import RRGraph from "@/app/components/heart/RRGraph";
...
export default function Page() {
  const [hrService, setHrService] = useState<
    BluetoothRemoteGATTService | undefined
  >(undefined);
  const [batService, setBatService] = useState<
    BluetoothRemoteGATTService | undefined
  >(undefined);
  ...
  async function handleConnectClick(evt: MouseEvent<HTMLButtonElement>) {
    ...
    try {
      const newDisService = await server?.getPrimaryService(
        "device_information",
      );
  ...
  const deviceDescription = disService?.device.gatt?.connected ? (
    <DeviceDesc service={disService} name={device?.name} id={device?.id} />
  ) : (
    <></>
  );
  const batteryStatus = batService?.device.gatt?.connected ? (
    <Battery service={batService} />
  ) : (
    <></>
  );
  const heartRate = hrService?.device.gatt?.connected ? (
    <HeartRate service={hrService} />
  ) : (
    <></>
  );
  const rrGraph = hrService?.device.gatt?.connected ? (
    <RRGraph service={hrService} />
  ) : (
    <></>
  );
